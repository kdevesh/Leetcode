...
class Node1 {
    ...
    Node1(int vertLevel, Node node, int horizontalLevel) {
        this.node = node;
...
public class BST {
    ...
    public List<List<Integer>> verticalTraversal(Node root) {
        ...
        Map<Integer, List<Node1>> mp = new TreeMap<>();
        int t = 0, level = 0;
        if (root == null)
            return res;
        queue.add(new Node1(0, root, 0));
        while (!queue.isEmpty()) {
            t = queue.size();
            level++;
            while (t != 0) {
                Node1 temp = queue.remove();
                List<Node1> lst = mp.getOrDefault(temp.vertLevel, new ArrayList<>());
                if (lst.size() != 0) {
                    int horlevel = temp.horizontalLevel;
                    int lstSize = lst.size() - 1;
                    List<Node1> tt = new LinkedList<>();
                    while (lstSize >= 0 && lst.get(lstSize).horizontalLevel == horlevel) {
                        tt.add(lst.remove(lstSize));
                        lstSize--;
                    }
                    tt.add(temp);
                    Collections.sort(tt, (x, y) -> x.node.data - y.node.data);
                    lst.addAll(tt);
                } else
                    lst.add(temp);
                mp.put(temp.vertLevel, lst);
                if (temp.node.left != null)
                    queue.add(new Node1(temp.vertLevel - 1, temp.node.left, level));
                if (temp.node.right != null)
                    queue.add(new Node1(temp.vertLevel + 1, temp.node.right, level));
                t--;
            }
        }
        for (Map.Entry<Integer, List<Node1>> entry : mp.entrySet()) {
            List<Integer> out = entry.getValue().stream().map(item -> item.node.data).collect(Collectors.toList());
            res.add(out);
        }
        return res;
    }
